...
const Authentication = () => {
  ...
  async function _setSwitches(macro_tutorial, micro_tutorial, materials) {
    try {
      await AsyncStorage.setItem(
        'macro_tutoriel',
        JSON.stringify(macro_tutorial)
      );
      await AsyncStorage.setItem(
        'micro_tutoriel',
        JSON.stringify(micro_tutorial)
      );
      await AsyncStorage.setItem('materials', JSON.stringify(materials));
    } catch (error) {
      console.log(
        "Tutorials and materials switches states couldn't be saved : ",
        error
      );
    }
  }
  ...
  const sendUserInfo = async (currentUser) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        await _setSwitches(
          response.data.macro_tutoriel,
          response.data.micro_tutoriel,
  ...
  const onLoggedIn = (loggedIn) => {
    if (loggedIn) router.replace('/(navbar)/Home/');
  };
  const loginText = "Se connecter / S'inscrire";
  const logoutText = 'Continuer';
    ...
    <View style={styles.view}>
      ...
      >
        <Text style={styles.title}>
          {' '}
          {loggedIn ? `Bienvenue ${user.name}` : 'Bienvenue'}{' '}
        </Text>
        <Button
          buttonColor="#00265A"
          contentStyle={styles.buttonRouter}
          style={styles.buttonRouter}
          onPress={() => (loggedIn ? onLoggedIn(loggedIn) : onLogin())}
        >
